...
class PointOfSell extends StatefulWidget {
  const PointOfSell({super.key, required this.data});
      ...
      body: SingleChildScrollView(
        child: MultiBlocProvider(
          ...
          child: Column(
            ...
            spacing: 5.h,
            children: [
              PosVendorInfo(data: widget.data),
              PosProductTable(data: widget.data),
              PosProductClickableSquares(data: widget.data),
              BlocBuilder<UpdateInvoiceCubit, UpdateInvoiceState>(
                ...
                builder: (context, state) {
                  ...
                  return state.data.isEmpty
                      ? SizedBox.shrink()
                      : PosProductTableInvoice(
                          productRowData: state.data,
                          //bnf: widget.data.bnf!,

                          onValideButtonPressed: () {
                            context.read<OtpBloc>().add(
                                SendOtp(phone: widget.data.bnf?.phone ?? ""));
                          },
                                      ...
                                      return AlertDialog(
                                          ...
                                          content: SendOtpDialogContent(
                                            onVerifyOtp: ({required otp}) {
                                              debugPrint(
                                                  'data.bnf?.phone = ${widget.data.bnf?.phone}');
                          ...
                          //     data.products!.fold<Map<ProductsEntity, int>>(
                          //   {},
                          //   (map, product) => map..[product] = Random().nextInt(100),
                          // ),
                        );
                },
              )
...
class PosProductClickableSquares extends StatelessWidget {
  const PosProductClickableSquares({
    super.key,
    required this.data,
  });
      ...
      child: SizedBox(
          ...
          child: ListView.separated(
            ...
            itemBuilder: (context, index) {
              num remainingQuota = (data.products![index].quotaMonth ?? 0) -
                  (data.products![index].usedQuota ?? 0);
              return Badge(
                backgroundColor: AppColors.green,
                  ...
                  onTap: remainingQuota > 0
                      ? () {
                          context
                              ...
                              .addNewRow(p: data.products![index]);
                        }
                      : null,
                  child: Card.outlined(
                    child: Container(
...
class PosVendorInfo extends StatelessWidget {
  const PosVendorInfo({
    super.key,
    required this.data,
  });
...
class PosProductTable extends StatelessWidget {
...
class PosProductTableInvoice extends StatefulWidget {
  const PosProductTableInvoice({
    super.key,
    required this.productRowData,
    required this.onValideButtonPressed,
    required this.showOtpValidationDialog,
    //required this.bnf,
  });

  final Map<ProductsEntity, num> productRowData;
  final VoidCallback onValideButtonPressed;
  final VoidCallback showOtpValidationDialog;

  @override
  State<PosProductTableInvoice> createState() => _PosProductTableInvoiceState();
}

class _PosProductTableInvoiceState extends State<PosProductTableInvoice> {
  // showAlertDialog(context,
  //     hasActions: true, title: "validation SMS",
  //     content: SendOtpDialogContent(
  //   onVerifyOtp: ({required otp}) {
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<OtpBloc, OtpState>(
      listener: (context, state) {
        ...
        if (state is SendOtpSuccess) widget.showOtpValidationDialog();
      },
      builder: (context, state) {
        return AbsorbPointer(
          absorbing: state is SendOtpLoading,
          child: Column(
            spacing: 15.h,
            children: [
              Card(
...
class OtpSMSField extends StatelessWidget {
  const OtpSMSField(
...
class SendOtpDialogContent extends StatefulWidget {
  const SendOtpDialogContent(
      {super.key, required this.onVerifyOtp, required this.state});

  final Function({required String otp}) onVerifyOtp;
  final OtpState state;

  @override
  State<SendOtpDialogContent> createState() => _SendOtpDialogContentState();
}

class _SendOtpDialogContentState extends State<SendOtpDialogContent> {
  ...
  Widget build(BuildContext context) {
    return Form(
        ...
        child: Column(
          ...
          children: [
            Text(
              AppLocalizations.of(context)!.sms_validation_message,
              //"Entrez le otp recu par SMS par le beneficiaire"
            ),
            OtpSMSField(
              controller: otpController,
              onChanged: (p0) => _validateForm(),
            ),
            widget.state is VerifyOtpFailed
                ? Padding(
                    padding: EdgeInsets.symmetric(vertical: 5.h),
                    child: Text(
                      (widget.state as VerifyOtpFailed).errorMessage,
                      style: TextStyle(
                  ...
                  child: CommonButton(
                    ...
                    fn: () {
                      if (formKey.currentState!.validate()) {
                        widget.onVerifyOtp(otp: otpController.text.trim());
                      }
                    },
                    isDisabled: _isButtonDisabled,
                    isLoading: widget is VerifyOtpLoading ||
                        widget.state is SaveSalesLoading,
                    color: AppColors.green,
                    textColor: AppColors.white,
                  ),
                ),
