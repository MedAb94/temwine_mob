                ...
                children: [
                          ...
                          children: [
                            ...
                            BlocConsumer<SyncBloc, SyncState>(
                              listener: (context, state) {
                                ...
                                } else if (state is SyncOfflineDataFailure) {
                                  showSellDialog(
                                    context,
                                    [state.errorMessage],
                                    true,
                  ...
                  TextField(
                    ...
                    decoration: InputDecoration(
                      labelText: AppLocalizations.of(context)!.benef_nni,
                    ),
                  ),
                  SizedBox(height: 10),
                  CommonButton(
                    ...
                    fn: () {
                      ...
                      if (nni.length == 10) {
                        final existing = context
                                ...
                                ?.beneficiaires ??
                            [];
                        if (existing.any((e) => e.nni == nni)) {
                          showSellDialog(
                            context,
                            [AppLocalizations.of(context)!.nni_already_exist],
                            true,
                          );
                          return;
                        }
                        context
                            .read<UserCubit>()
                            .addBeneficiaire(beneficiaire: BnfEntity(nni: nni));
                        _nniController.clear();
                        showSellDialog(
                          context,
                          [AppLocalizations.of(context)!.nni_saved_success],
                        );
                      }
                    },
                  ),
    ...
    return BlocBuilder<LanguageCubit, Locale?>(
      builder: (context, state) {
        _selectedValue = map.entries
            .firstWhere((entry) => entry.value == state?.languageCode)
            .key;
